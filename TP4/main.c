...
static int verifier_abr_aux(ab_t a, int min, int max) {
	...
	return verifier_abr_aux(a->gauche, min, x->e - 1) && verifier_abr_aux(a->droit, x->e + 1, max);
}
...
int main() {
	...
	for (int type_idx = 0 ; type_idx < nb_types ; type_idx++) {
		...
		for (int i = 0 ; i < nb_sizes ; i++) {
			...
			/* Sauvegarde dans CSV */
			fprintf(f, "%d,%s,%d,%d,%.2f,%.2f,%.2f,%.2f\n", n, types[type_idx], hauteur_eq, hauteur_seq, temps_insertion_eq, temps_in	sertion_seq, temps_recherche_eq, temps_recherche_seq);
	...
	printf("Executez 'python3 analyse_abr.py' pour visualiser les courbes.\n");
